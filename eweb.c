...
void write_header(int socket_fd, char *head, long content_len) {
	...
	string_add(header, "\nContent-Length: ");
	char cl[10] = { 0 }; // 100Mb = 104,857,600 bytes
...
void webhit(struct hitArgs *args) {
	...
	struct http_header content_length = get_header("Content-Length", string_chars(args->buffer), args->buffer->used_bytes);
	args->content_length = atoi(content_length.value);
	const long body_start = get_body_start(string_chars(args->buffer));
	...
	const http_verb type = request_type(string_chars(args->buffer));
	...
	char *body = (type == HTTP_GET) ? NULL : (char*)args->buffer->ptr + get_body_start(string_chars(args->buffer));
