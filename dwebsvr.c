...
void webhit(struct hitArgs *args) {
	...
	args->logger_function(LOG, "request", string_chars(args->buffer),
			      args->hit);

	if (type =
	    request_type(string_chars(args->buffer)),
	    type == HTTP_NOT_SUPPORTED) {
		forbidden_403(args,
			      "Only simple GET and POST operations are supported");
		finish_hit(args, 3);
		return;
	}
	// get a pointer to the request body (or NULL if it's not there)
	body = (type == HTTP_GET) ? NULL : (char*)args->buffer->ptr + get_body_start(string_chars(args->buffer));
	...
	args->responder_function(args,
				 string_chars(args->buffer) +
				 ((type == HTTP_GET) ? 5 : 6), body, type);
	finish_hit(args, 1);
}
...
int dwebserver(int port, void (*responder_func) (struct hitArgs * args, char *, char *, http_verb),	// pointer to responder func
